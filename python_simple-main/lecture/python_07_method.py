...
#   + 함수 내용은 가능하면 짧게 작성(Code Lime 줄이기)
#   ex) def print_hello():
#           print("hello")

# 2. 함수의 역할
#   + 하나의 함수에 유사한 역할을 하른 코드만 포함
...
# 3.인자(Parametter or arguement)
# - 함수에 전달되는 입력값(input)
# - 함수 정의문과 호출문의 parameter 갯수가 동일해야함
# - parameter로 int, str, float, bool, list 사용가능
# - 심지어 사용자 정의 함수를 parameter로 전달 가능
# - parameter값 2개 이상 사용시 정의된 순서대로 전달해야 함


def sub_two_value(x: int, y: int): #hint
    n = x-y
    return n
a, b = 15, 20
...
# 4.Default Parameter
# - 상수 호출시 parameter를 전달 받지 못한 경우 기본값 사용
